...
def AnalisisBivariado(st, session):
    ...
    with c1:
        ...
        fig = px.box(df, x='sexo', y='satisfaccion_general_univalle', title='Comparación de la Satisfacción General con Univalle por Sexo')
        st.plotly_chart(fig)
    with c2:
        st.write("### Relación entre la edad y la satisfacción con la formación académica")
        fig = px.scatter(df, x='edad', y='satisfaccion_formacion_academica', title='Relación entre Edad y Satisfacción con la Formación Académica')
        st.plotly_chart(fig)
    c3,c4=st.columns(2)
    with c3:
        st.write("### Comparación de la satisfacción con los docentes por carrera")
        fig = px.box(df, x='carrera', y='satisfaccion_docentes', title='Comparación de la Satisfacción con los Docentes por Carrera')
        st.plotly_chart(fig)
    with c4:
        st.write("### Relación entre el trabajo actual y la satisfacción con la carrera")
        fig = px.box(df, x='trabaja_actualmente', y='satisfaccion_carrera', title='Relación entre el Trabajo Actual y la Satisfacción con la Carrera')
        st.plotly_chart(fig)
    c5,c6=st.columns(2)
    with c5:
        st.write("### Comparación de la satisfacción con la infraestructura por sede")
        fig = px.box(df, x='sede', y='satisfaccion_infraestructura', title='Comparación de la Satisfacción con la Infraestructura por Sede')
        st.plotly_chart(fig)
    with c6:
    ...
    with c8:
        ...
        fig = px.box(df, x='ciudad', y='carrera', title='Relación entre la Ciudad de Procedencia y la Carrera')
        st.plotly_chart(fig)
        
    st.write("### Comparación de la satisfacción con la atención administrativa por si trabaja actualmente o no")
    fig = px.box(df, x='trabaja_actualmente', y='satisfaccion_atencion_administrativa', title='Comparación de la Satisfacción con la Atención Administrativa por Si Trabaja Actualmente o No')
    st.plotly_chart(fig)
